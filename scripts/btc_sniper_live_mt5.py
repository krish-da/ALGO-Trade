#!/usr/bin/env python3
"""
BTC SNIPER - LIVE TRADING VERSION (MT5)
========================================
EXACT SAME LOGIC AS BACKTEST - Live execution on MT5 Funding Pips Tournament

5-minute analysis, 1-minute execution, sniper entries
Dynamic TP extensions, Aggressive trailing
85% win rate, +1588% ROI (based on backtest)
"""
...
class BTCSniperLive:
    def __init__(self, account_size=10000, phase='phase1', symbol='BTCUSD'):
        ...
        # ENTRY LOGIC - SNIPER STYLE (EXACT SAME AS BACKTEST)
        ...
        # EXECUTION - 1-MIN PRECISION (EXACT SAME AS BACKTEST)
        ...
        self.tp_extension_distance = self.breakout_threshold * 2  # 80 pips
        self.max_tp_extensions = 3  # Fewer extensions = take profit faster
        
        # AGGRESSIVE TRAILING (EXACT SAME AS BACKTEST)
        self.trail_activation_rr = 1.8       # Trail later = more profit
        self.trail_lock_pct = 0.7            # Lock more profit (70%)
        self.trail_step_pips = self.breakout_threshold * 0.5  # 20 pips
        ...
        # TRADE FILTERING (EXACT SAME AS BACKTEST)
        self.min_5m_candles_between = 8  # Allow more trades (40 minutes)
        ...
        print(f"\n{'='*80}")
        print(f"BTC SNIPER - LIVE TRADING (EXACT BACKTEST LOGIC)")
        print(f"{'='*80}")
        ...
        print(f"BTC Effective Leverage: 1:{int(self.leverage)}")
        print(f"Risk: {self.risk_pct}% per trade")
        print(f"Expected: 85% win rate, +1588% ROI (from backtest)")
        print(f"{'='*80}\n")
    ...
    def _get_funding_pips_rules(self):
        """Phase rules for reference only - NOT ENFORCED"""
        rules = {
            'phase1': {'profit_target_pct': 8.0},
            'phase2': {'profit_target_pct': 5.0},
            'master': {'profit_target_pct': 10.0}
        }
    ...
    def _enter_trade(self, direction, entry, sl, tp, level, is_confluence):
        ...
        # Prepare MT5 order - REAL EXECUTION
        ...
        request = {
            ...
            "deviation": 50,  # More slippage tolerance
            "magic": 234567,  # BTC magic number
            "comment": "BTC_Sniper",
            "type_time": mt5.ORDER_TIME_GTC,
            "type_filling": mt5.ORDER_FILLING_IOC,
        }
        
        print(f"\n🔥 EXECUTING REAL ORDER ON MT5:")
        ...
        if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
            ...
            # Try with different fill type if IOC failed
            if result and result.retcode == 10030:  # Invalid fills
                print(f"   Retrying with FOK fill type...")
                request["type_filling"] = mt5.ORDER_FILLING_FOK
                result = mt5.order_send(request)
                
                if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
                    print(f"❌ Still failed: {result.comment if result else mt5.last_error()}")
                    return False
            else:
                return False
        
        print(f"✅ ORDER EXECUTED ON MT5!")
        ...
        self.active_trade = {
            'ticket': result.order,
            'dir': direction,
            'entry': result.price,  # Use actual execution price
            'sl': sl,
            'tp': tp,
            'initial_tp': tp,
            'tp_extensions': 0,
        ...
        rr = abs(tp - result.price) / sl_dist
        
        print(f"\n{'='*70}")
        print(f"🎯 {direction} POSITION OPENED @ {confluence_str}")
        print(f"   Entry: ${result.price:,.2f} | SL: ${sl:,.2f} ({sl_dist:.0f}p)")
        print(f"   TP: ${tp:,.2f} | R:R = 1:{rr:.1f}")
        print(f"   Level: ${level:,.2f} | Lots: {lots} | Ticket: {result.order}")
        print(f"   LIVE ON MT5 - REAL TRADE!")
        print(f"{'='*70}")
                    ...
                    if not self.active_trade:
                        ...
                        if df_5m is not None and len(df_5m) > 0:
                            ...
                            if len(df_5m) >= 2:
                                ...
                                print(f"   Nearest Zone: ${nearest_zone:,.2f} ({nearest_dist:.0f} pips away)")
                                ...
                                if nearest_dist <= self.zone_proximity_5m:
                                    ...
                                    if completed_close > recent_high + self.breakout_threshold:
                                        print(f"   🚀 BULLISH BREAKOUT! ${(completed_close - recent_high):.0f} pips above high")
                                    elif completed_close < recent_low - self.breakout_threshold:
                                        print(f"   🔻 BEARISH BREAKOUT! ${(recent_low - completed_close):.0f} pips below low")
                                    else:
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("BTC SNIPER - LIVE TRADING (MT5 Funding Pips Tournament)")
