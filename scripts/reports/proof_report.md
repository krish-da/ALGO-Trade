...
## Addendum: "synthetic data" concern — resolved with real spot data

The user questioned whether the proof used synthetic data. **It did not.** Two checks
were run (`scripts/prove_edge_spot.py`):

### A. Futures proxy validation against the user's own broker feed

The MT5 spot cache timestamps are broker server time, offset **-1 hour** from UTC.
After aligning:

| Metric | Value |
|---|---|
| 15m log-return correlation (GC=F vs user's spot) | **0.9817** |
| Basis (GC=F − spot) | mean $15.66, stdev **$3.60** |

GC=F is real exchange-traded gold and moves essentially identically to the user's
spot feed (a near-constant premium). The 1-year walk-forward result stands: the
time offset never affected it, since that test was internally consistent on one feed.

### B. Full proof re-run on ONLY the user's real MT5 spot data (25 days, 15m)

| Metric | Value |
|---|---|
| Trades (walk-forward, zero lookahead) | 166 |
| Win rate | 37.3% |
| Avg R | **+0.321** |
| Total P&L | **+$346/oz** |
| Avg win / avg loss | $12.6 / $4.2 (3.0x asymmetry) |
| Random-level Monte Carlo (300 runs) | +0.019 ± 0.408 |
| **Z-score** | **+0.74** (needed ≥ 2.0; empirical p = 0.18) |

So on the user's own data and recent period, the method **was genuinely profitable** —
the user's experience is real, not imagined. But: (1) 18% of randomly-placed-line
runs did just as well in this window, and (2) the 13-month test shows this exact
period (Jun–Jul 2026) was an outlier regime. Profitable recent weeks + zero
13-month expectancy = regime-dependent edge, not a permanent one. The NO-GO verdict
for the *unfiltered* strategy stands; a regime-filtered variant remains the most
promising next hypothesis.

---

*Generated by `scripts/prove_edge.py` (walk-forward, zero lookahead),
`scripts/prove_edge_spot.py` (spot-only re-run + proxy validation), and
